 ...
 * - never their domain/data headers directly, same rule every feature's own
 * domain layer states about itself. Unlike ied_model/mms_report_client/
 * goose_subscriber, this layer has zero direct third-party includes of its
 * own (only transitively, via the sibling _api.h headers) - orchestration's
 * job is sequencing already-implemented features, not talking to
 * libiec61850 itself.
 */
...
struct sOrchestrationHandle {
    ...
    GooseSubscriberCallback gooseRecordCallback;
    void* gooseRecordCallbackParam;
