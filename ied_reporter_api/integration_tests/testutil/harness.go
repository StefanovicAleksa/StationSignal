...
// Harness is a running ied_reporter_api process, wired to a real daemon binary, with small
...
// StartAPI builds and spawns ied_reporter_api against daemonBin, waits for it to report
...
// PID returns the OS process ID of the ied_reporter_api process itself.
...
// daemon child and waits for it to actually exit — see cmd/ied_reporter_api/main.go).
...
// consecutive polls (not just once) before returning — two ied_reporter_daemon processes
