...
## Purpose
The Go service between the frontend and `ied_reporter_daemon` (see `../CLAUDE.md` for how this
fits into the whole product). Two jobs:

1. **Supervise the daemon** — start/stop/monitor the `ied_reporter_daemon` process, and own its
...
This file governs `ied_reporter_api/` only. `../ied_reporter_daemon/CLAUDE.md` remains the
...
Summarized from `../ied_reporter_daemon/docs/AGENT_API_GUIDE.md` — that file is the full,
...
- **Build vs. prebuilt binary** — does this API expect an already-built `ied_reporter_daemon`
