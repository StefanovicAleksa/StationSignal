 ...
 * in deliberately-failing, no-network cases (empty host, unknown deviceId) -
 ...
 * mapping (a real StartReporting succeeding) is proven only by
 * integration_tests/control_dispatcher/'s E2E test, which has a real IED to
 * talk to.
 */
...
test_processRequest_returnsNull_onNullRequest(void) {
    TEST_ASSERT_NULL(ControlDispatcherUseCases_processRequest(NULL, fixtureDeviceManager));
}
...
test_startReporting_invalidArgument_mapsToErrorEnvelope(void) {
    ...
    fixtureJson = ControlDispatcherUseCases_processRequest(request, fixtureDeviceManager);
    freeRequest(request);
