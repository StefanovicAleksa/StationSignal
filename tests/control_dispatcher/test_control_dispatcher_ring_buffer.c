...
static ControlDispatcherRingBuffer fixtureBuffer;
...
tearDown(void) {
    if (fixtureBuffer) {
        ControlDispatcherRingBuffer_destroy(fixtureBuffer);
        fixtureBuffer = NULL;
    }
}
...
test_create_returnsNull_onInvalidCapacity(void) {
    TEST_ASSERT_NULL(ControlDispatcherRingBuffer_create(0));
    TEST_ASSERT_NULL(ControlDispatcherRingBuffer_create(-1));
}

void
test_headSeq_startsAtZero(void) {
    fixtureBuffer = ControlDispatcherRingBuffer_create(4);
    TEST_ASSERT_EQUAL_UINT64(0, ControlDispatcherRingBuffer_headSeq(fixtureBuffer));
}

void
test_readNext_returnsNull_whenCursorAtHead(void) {
    fixtureBuffer = ControlDispatcherRingBuffer_create(4);
    uint64_t cursor = ControlDispatcherRingBuffer_headSeq(fixtureBuffer);

    char* result = ControlDispatcherRingBuffer_readNext(fixtureBuffer, &cursor, NULL);
...
test_push_thenReadNext_returnsCopy_notAlias(void) {
    ...
    ControlDispatcherRingBuffer_push(fixtureBuffer, dupStr("{\"a\":1}"));

    uint64_t dropped = 999;
    char* result = ControlDispatcherRingBuffer_readNext(fixtureBuffer, &cursor, &dropped);
    ...
    TEST_ASSERT_NULL(ControlDispatcherRingBuffer_readNext(fixtureBuffer, &cursor, NULL));
}

void
test_wraparound_dropsOldest(void) {
    fixtureBuffer = ControlDispatcherRingBuffer_create(2);

    ControlDispatcherRingBuffer_push(fixtureBuffer, dupStr("msg0"));
    ControlDispatcherRingBuffer_push(fixtureBuffer, dupStr("msg1"));
    ControlDispatcherRingBuffer_push(fixtureBuffer, dupStr("msg2")); /* overwrites msg0's slot */
    ...
    char* second = ControlDispatcherRingBuffer_readNext(fixtureBuffer, &cursor, &dropped);
...
test_twoIndependentCursors_laggingOneDoesNotAffectCaughtUpOne(void) {
    ...
    uint64_t caughtUpCursor = ControlDispatcherRingBuffer_headSeq(fixtureBuffer);
    uint64_t laggingCursor = caughtUpCursor;

    ControlDispatcherRingBuffer_push(fixtureBuffer, dupStr("msg0"));

    /* caughtUp reads immediately */
    char* r1 = ControlDispatcherRingBuffer_readNext(fixtureBuffer, &caughtUpCursor, NULL);
    TEST_ASSERT_EQUAL_STRING("msg0", r1);
    free(r1);

    /* more pushes happen before lagging reader catches up, past capacity */
    ControlDispatcherRingBuffer_push(fixtureBuffer, dupStr("msg1"));
    ControlDispatcherRingBuffer_push(fixtureBuffer, dupStr("msg2"));

    /* caughtUp reader sees the rest in order, no drops (never fell behind) */
    uint64_t droppedForCaughtUp = 999;
    char* r2 = ControlDispatcherRingBuffer_readNext(fixtureBuffer, &caughtUpCursor, &droppedForCaughtUp);
    ...
    char* r3 = ControlDispatcherRingBuffer_readNext(fixtureBuffer, &laggingCursor, &droppedForLagging);
...
test_push_takesOwnership_evenOnNullBuffer(void) {
    ...
    ControlDispatcherRingBuffer_push(NULL, dupStr("discarded"));
}

void
test_destroy_doesNotCrash_onNull(void) {
    ControlDispatcherRingBuffer_destroy(NULL);
}
