...
// DaemonPID finds the real station_signal_daemon process's PID — the API execs it as a direct
// child (no intermediate shell), so `pgrep -P <api pid>` finds exactly it. Retries briefly:
// two station_signal_daemon processes sequentially reusing the same fixed control-channel port
