...
func fakeSpawn(t *testing.T, spawnErrs []error, exitChs []chan error) (spawnFn func(string, string, string, *slog.Logger) (*data.Process, <-chan error, error), calls func() int) {
	t.Helper()
	var n int32
	fn := func(string, string, string, *slog.Logger) (*data.Process, <-chan error, error) {
...
func newTestSupervisor(spawnFn func(string, string, string, *slog.Logger) (*data.Process, <-chan error, error), pollReadyFn func(context.Context, string, time.Duration, time.Duration) bool) *Supervisor {
	return &Supervisor{
		binPath:     "unused",
		controlAddr: "unused",
		logger:      slog.Default(),
...
func TestSupervisor_Running_FalseBeforeAnySpawn(t *testing.T) {
	s := newTestSupervisor(func(string, string, string, *slog.Logger) (*data.Process, <-chan error, error) {
