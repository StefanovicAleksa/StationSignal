# ied_reporter_api — API Guide for the Frontend

This is a self-contained reference for building a frontend against a running
`ied_reporter_api` instance. It assumes no access to this repo's source — everything here is
...
A REST + WebSocket API that supervises a real `ied_reporter_daemon` process and relays
...
The API is a single process, `ied_reporter_api`, that takes a required `-daemon-bin` flag (or
`IED_REPORTER_DAEMON_BIN` env var) pointing at a prebuilt daemon binary, and listens on
...
- `controlChannelConnected` — the API's connection to the daemon's control channel is up,
  i.e. `/devices` and `/scans` calls will actually be able to reach the daemon right now.
...
## 2. Device reporting — `/devices`

### `POST /devices` — start reporting on one IED
...
| `sclFilePath` | no | a path on the **daemon's own filesystem**. Either a path you already know exists there, or — more commonly — the `path` returned by `POST /structure-files` (§2a) after uploading a file from the browser |
...
### `GET /devices` — list every currently active device
...
### `POST /structure-files` — upload an SCL/ICD/CID structure file
...
```json
{ "path": "/var/lib/ied_reporter_api/structure_files/3f9a1c2d-device.icd" }
```

Pass this `path` straight through as `sclFilePath` in a subsequent `POST /devices` call — it's
...
### `POST /scans` — start a background subnet sweep
...
data, and can arrive **after** `POST /devices` already returned `201` — MMS report connections
...
- The connection **closes** if the device is stopped (`DELETE /devices/{id}`) or the daemon
  crashes and hasn't been re-armed yet — reconnect and re-check `GET /devices` if you need to
...
about its data model has been fetched. To actually report on it, `POST /devices` with its
`host`/`mmsPort`.
...
| 503 | `DAEMON_UNREACHABLE` | this API couldn't reach the daemon at all (down, restarting, or the call timed out) — check `GET /health` |
...
receiving it, prompt the user for a password and retry the same `POST /devices` call with
...
- **`wsPort` in a `POST /devices` response is not for you.** It's an internal daemon detail;
