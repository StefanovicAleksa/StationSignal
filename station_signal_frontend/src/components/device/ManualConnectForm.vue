  ...
  <form class="flex flex-wrap items-end gap-4" @submit.prevent="handleSubmit">
    ...
    <div class="flex flex-col gap-1">
      ...
      <input
        ...
        :disabled="props.disabled"
        class="w-36 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none disabled:bg-slate-100 disabled:text-slate-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:disabled:bg-slate-800 dark:disabled:text-slate-500"
      />
    </div>
