...
- **`station_signal_daemon/`** — implemented. C. Sniffs GOOSE off the wire and subscribes to MMS
  ...
  discovered-device results). See `station_signal_daemon/CLAUDE.md` for the full command/websocket
  contract and internal architecture.
- **`station_signal_api/`** — implemented. Go. Two jobs: supervise the daemon process (start/
  ...
  See `station_signal_api/CLAUDE.md` and `station_signal_api/docs/API_OVERVIEW.md` for internals,
  and `station_signal_api/docs/FRONTEND_API_GUIDE.md` for the wire contract the frontend builds
  against.
- **`station_signal_frontend/`** — scaffolded, no application code yet. Vue 3 + TypeScript + Vue
  Router + Pinia + Vitest. What the technician actually looks at on-site: live GOOSE/MMS data,
  device/scan controls. See `station_signal_frontend/CLAUDE.md`.
...
## Current State
- **Daemon**: fully implemented and proven end-to-end (see `station_signal_daemon/CLAUDE.md` and its
...
- **Frontend**: implemented (Vue 3 + TypeScript + Vue Router + Pinia + Vitest) — devices, scan,
  reports, and settings views are built out, localized (English + Serbian, see that file's own
  i18n section); see `station_signal_frontend/CLAUDE.md`.
