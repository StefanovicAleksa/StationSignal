...
`../CLAUDE.md` for how this fits into the whole product). Talks to `ied_reporter_api` only —
never to `ied_reporter_daemon` directly, and never over anything but the REST/WS surface that
API exposes.

This file governs `frontend/` only. `../ied_reporter_api/CLAUDE.md` remains the authority on
...
Read `../ied_reporter_api/docs/FRONTEND_API_GUIDE.md` **before writing any client code** — it
...
- **Never connect to the daemon's own ports.** This frontend's only backend is
  `ied_reporter_api` — even though the daemon's raw wire format is technically reachable if
...
- **Treat `FRONTEND_API_GUIDE.md` as an external stable contract to consume, not guess at.**
  If the API's response shape ever seems to need a change, that's a `ied_reporter_api` change
