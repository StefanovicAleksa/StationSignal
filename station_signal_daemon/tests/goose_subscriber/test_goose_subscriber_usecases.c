...
/* ---- filterRecentForwardDuplicates (recent-forward duplicate-content suppression) ----
 *
 * GooseSubscriberUseCases_filterRecentForwardDuplicates filters its `entries` array IN
 * PLACE and returns the surviving count - dropped entries are freed internally by the
 * call itself. Every test below must free only the RETURNED count, not the count it
 * originally passed in, or it will double-free entries the call already consumed. */
...
static GooseSubscriberEntry*
makeRecentForwardEntry(const char* ref0, bool val0) {
    GooseSubscriberEntry* entries = calloc(1, sizeof(GooseSubscriberEntry));
    entries[0].reference = strdup(ref0);
    entries[0].value = MmsValue_newBoolean(val0);
    return entries;
}
...
void
test_filterRecentForwardDuplicates_firstEverContent_isForwarded_andSeedsCache(void) {
    ...
    int result = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbA", entries, 2);

    TEST_ASSERT_EQUAL_INT_MESSAGE(2, result, "nothing cached yet - must always forward every entry");
    TEST_ASSERT_EQUAL_INT(2, cache.count);
    TEST_ASSERT_EQUAL_STRING("LD/LLN0$GO$gcbA", cache.history[0].goCbRef);
    TEST_ASSERT_EQUAL_STRING("LD/LLN0$GO$gcbA", cache.history[1].goCbRef);

    freeRecentForwardEntries(entries, result);
    GooseSubscriberUseCases_destroyRecentForwardCache(&cache);
}

void
test_filterRecentForwardDuplicates_sameTargetIdenticalContent_isStillForwarded(void) {
     ...
     * Mirrors mms_report_client's identical same-RCB behavior exactly. */
    GooseSubscriberRecentForwardCache cache = { 0 };
    GooseSubscriberEntry* entries1 = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int result1 = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbA", entries1, 2);
    TEST_ASSERT_EQUAL_INT(2, result1);

    GooseSubscriberEntry* entries2 = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int result2 = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbA", entries2, 2);

    TEST_ASSERT_EQUAL_INT(2, result2);

    freeRecentForwardEntries(entries1, result1);
    freeRecentForwardEntries(entries2, result2);
    GooseSubscriberUseCases_destroyRecentForwardCache(&cache);
}

void
test_filterRecentForwardDuplicates_differentTargetIdenticalContent_isSuppressed(void) {
    GooseSubscriberRecentForwardCache cache = { 0 };
    GooseSubscriberEntry* entries1 = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int result1 = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbA", entries1, 2);
    ...
    GooseSubscriberEntry* entries2 = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int result2 = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbB", entries2, 2);

    TEST_ASSERT_EQUAL_INT_MESSAGE(0, result2,
            "a different GoCB reporting byte-identical content must be fully suppressed as a duplicate, "
            "regardless of its own wire timestamp - independent GoCBs stamp `t` independently and "
            "can legitimately differ even for the same underlying event");
...
void
test_filterRecentForwardDuplicates_differentTargetDifferentContent_isForwarded(void) {
    GooseSubscriberRecentForwardCache cache = { 0 };
    GooseSubscriberEntry* entries1 = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int result1 = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbA", entries1, 2);
    ...
    /* Both values must genuinely differ from entries1's, not just one - a
     * value that happens to coincidentally match entries1's own (even at a
     * different reference) is now correctly caught by the finer per-entry
     * match this fix introduces. */
    GooseSubscriberEntry* entries2 = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", false, "LD/GGIO1$ST$SPCSO5$stVal", false);
    int result2 = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbB", entries2, 2);

    TEST_ASSERT_EQUAL_INT_MESSAGE(2, result2, "genuinely different content from a different GoCB must be forwarded");
    TEST_ASSERT_EQUAL_INT(4, cache.count);
    TEST_ASSERT_EQUAL_STRING("LD/LLN0$GO$gcbB", cache.history[2].goCbRef);
...
void
test_filterRecentForwardDuplicates_suppressionDoesNotDisturbEstablishedHistory(void) {
    GooseSubscriberRecentForwardCache cache = { 0 };
    GooseSubscriberEntry* entriesA = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int resultA = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbA", entriesA, 2);
    TEST_ASSERT_EQUAL_INT(2, resultA);

    GooseSubscriberEntry* entriesB = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int resultB = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbB", entriesB, 2);
    TEST_ASSERT_EQUAL_INT(0, resultB);

    GooseSubscriberEntry* entriesC = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int resultC = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbC", entriesC, 2);

    TEST_ASSERT_EQUAL_INT_MESSAGE(0, resultC, "C must still be recognized as a duplicate of A's original content, "
            "even though B's suppressed entries never touched the cache");
    TEST_ASSERT_EQUAL_INT_MESSAGE(2, cache.count,
            "the cache must still hold only A's entries, the only ones actually forwarded");
    ...
    freeRecentForwardEntries(entriesA, resultA);
    freeRecentForwardEntries(entriesB, resultB);
    freeRecentForwardEntries(entriesC, resultC);
    GooseSubscriberUseCases_destroyRecentForwardCache(&cache);
}

void
test_filterRecentForwardDuplicates_interleavedUnrelatedGoCb_stillCatchesDuplicate(void) {
    ...
    int resultC = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbC", entriesC, 2);
    TEST_ASSERT_EQUAL_INT_MESSAGE(2, resultC, "gcbC's genuinely unrelated content must be forwarded normally");

    GooseSubscriberEntry* entriesB = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int resultB = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbB", entriesB, 2);

    TEST_ASSERT_EQUAL_INT_MESSAGE(0, resultB,
...
void
test_filterRecentForwardDuplicates_isNoOp_whenCacheIsNull(void) {
    GooseSubscriberEntry* entries = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO5$stVal", true);
    int result = GooseSubscriberUseCases_filterRecentForwardDuplicates(NULL, "LD/LLN0$GO$gcbA", entries, 2);
    TEST_ASSERT_EQUAL_INT(2, result);
    freeRecentForwardEntries(entries, result);
}

void
test_filterRecentForwardDuplicates_partiallyOverlappingDatasets_onlySharedEntryDropped(void) {
    /* Reproduces the bug this fix targets: two GoCBs whose datasets share
     * one point (stVal) but aren't shape-identical - gcbB's dataset also
     * carries its own `q` sibling gcbA's dataset doesn't have. The old
     * whole-record positional comparison required an exact entry-count
     * match, so it never recognized these as sharing anything; only the
     * genuinely-shared stVal entry should be dropped here, and the unique q
     * entry must survive. */
    GooseSubscriberRecentForwardCache cache = { 0 };

    GooseSubscriberEntry* entriesA = makeRecentForwardEntry("LD/GGIO1$ST$SPCSO4$stVal", false);
    int resultA = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbA", entriesA, 1);
    TEST_ASSERT_EQUAL_INT(1, resultA);

    GooseSubscriberEntry* entriesB = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", false, "LD/GGIO1$ST$SPCSO4$q", true);
    int resultB = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbB", entriesB, 2);

    TEST_ASSERT_EQUAL_INT_MESSAGE(1, resultB,
            "only the shared stVal entry should be dropped as a cross-GoCB duplicate - the unique q entry must survive");
    TEST_ASSERT_EQUAL_STRING_MESSAGE("LD/GGIO1$ST$SPCSO4$q", entriesB[0].reference,
            "the surviving entry after in-place compaction must be the unique q entry, not the dropped stVal");
...
void
test_filterRecentForwardDuplicates_reorderedIdenticalDatasets_stillFullyDeduped(void) {
    /* gcbA's dataset is [stVal, q]; gcbB's carries the exact same two
     * values but in the OPPOSITE member order: [q, stVal]. Real SCL can
     * configure two GoCBs covering the same point with differently-ordered
     * datasets. The old positional comparison (index-for-index) would have
     * wrongly forwarded this as "different content", even though this case
     * is simpler than the partial-overlap one above - both datasets are the
     * same size and share every member, just reordered. */
    GooseSubscriberRecentForwardCache cache = { 0 };

    GooseSubscriberEntry* entriesA = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$stVal", true, "LD/GGIO1$ST$SPCSO4$q", false);
    int resultA = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbA", entriesA, 2);
    TEST_ASSERT_EQUAL_INT(2, resultA);

    GooseSubscriberEntry* entriesB = makeRecentForwardEntries(
            "LD/GGIO1$ST$SPCSO4$q", false, "LD/GGIO1$ST$SPCSO4$stVal", true);
    int resultB = GooseSubscriberUseCases_filterRecentForwardDuplicates(&cache, "LD/LLN0$GO$gcbB", entriesB, 2);

    TEST_ASSERT_EQUAL_INT_MESSAGE(0, resultB,
            "reordered-but-identical content from a different GoCB must be fully deduped regardless of member order");
...
main(void) {
    ...
    RUN_TEST(test_filterRecentForwardDuplicates_firstEverContent_isForwarded_andSeedsCache);
    RUN_TEST(test_filterRecentForwardDuplicates_sameTargetIdenticalContent_isStillForwarded);
    RUN_TEST(test_filterRecentForwardDuplicates_differentTargetIdenticalContent_isSuppressed);
    RUN_TEST(test_filterRecentForwardDuplicates_differentTargetDifferentContent_isForwarded);
    RUN_TEST(test_filterRecentForwardDuplicates_suppressionDoesNotDisturbEstablishedHistory);
    RUN_TEST(test_filterRecentForwardDuplicates_interleavedUnrelatedGoCb_stillCatchesDuplicate);
    RUN_TEST(test_filterRecentForwardDuplicates_isNoOp_whenCacheIsNull);
    RUN_TEST(test_filterRecentForwardDuplicates_partiallyOverlappingDatasets_onlySharedEntryDropped);
    RUN_TEST(test_filterRecentForwardDuplicates_reorderedIdenticalDatasets_stillFullyDeduped);
